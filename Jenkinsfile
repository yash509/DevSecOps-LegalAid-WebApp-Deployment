pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/legalaid-webapp"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-LegalAid-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop legalaid-webapp | true"
                sh "docker rm legalaid-webapp | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name legalaid-webapp -p 3000:3000 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service legalaid-webapp-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"legalaid-webapp\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
